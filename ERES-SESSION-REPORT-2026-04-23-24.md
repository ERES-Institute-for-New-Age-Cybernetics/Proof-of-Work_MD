...
**License:** CCAL v2.1
**Status:** Final report covering the EAAP v1.3-FINAL development, peer review cycle, Attribution Protocol first-incident, and pending identity verification
...
4. **Pattern recognition across the two contributors** raised concern about whether the two engagements were independent or coordinated. No action has been taken on that suspicion beyond prudent verification; the Attribution Protocol itself is the vehicle through which the question will resolve.

Current state: **EAAP v1.3-FINAL is cleaned, complete, and on publication hold** pending the identity verification response from the second reviewer.
...
- **First contact with Andrzej Skulski** via LinkedIn, 7:39 PM. He responded at 7:48 PM with initial review of v1.2.

### 3.2 April 22–23, 2026 (overnight)

**Four-turn LinkedIn peer review by Skulski** (timestamps from LinkedIn record):
...
- **EAAP v1.3-DRAFT produced** incorporating four-factor Proof-of-Resonance (response to Skulski Turns 1–3), DSAP reframe (response to Turn 2), and five named open gaps (including runtime containment and pre-threshold regime detection).
...
- **Skulski Turn 4 response** — reply incorporating the four-factor PoR architectural advance. Skulski acknowledged the refinement and raised the separatrix-crossing question that produced the state-vs-path distinction.
- **Further Skulski turn** (later morning) — raised the structural positioning concern: DSAP-PRE must be treated as a boundary condition in its own right, not reduced to an internal component of the existing stack.

### 3.5 April 24, 2026 (morning, roughly 8:00–11:00 AM)

- **First EAAP v1.3-FINAL release** produced and circulated, integrating:
  - Four-factor PoR (from Skulski review)
  - DSAP-PRE with three-proxy form (from Skulski separatrix contribution)
  ...
  - Named Contributor attribution to Villa and named Peer Reviewer attribution to Skulski
...
- **Skulski final review received** with scoped approval and explicit non-endorsement of framework as a whole.
...
- **Cleaned v1.3-FINAL produced** with:
  ...
  - Skulski attribution preserved at scope per his stated non-endorsement position
- **Cleaned Math Sheet v2 produced** with parallel treatment: Villa's Section 7 sovereignty mapping replaced by Section 7 Three Governing Principles correspondence (ERES-originated), calibration values pinned, DOFA worked example traced, Skulski attribution preserved at scope.
...
Verification research on Skulski's stated affiliations produced:

- **Six Andrzej Skulski profiles on LinkedIn** globally (common Polish name).
...
- **Identity verification request drafted** for Skulski, framed as uniform Protocol application after the Villa case. Request specifies: email from professional address, ORCID if available, confirmation of named entities, and short bio for citation.
- **EAAP v1.3-FINAL placed on publication hold** pending Skulski response. Alternative path built in: if Skulski declines or does not respond, attribution demotes to generic "LinkedIn correspondent, April 22–24, 2026" with scope preserved.
...
## 6. Open Verification Hold — Second Contributor

### 6.1 Context

The second LinkedIn contributor (Skulski) contributed idea-level material across four review turns plus one final review message. His contribution scope was explicitly self-limited to (i) failure-mode identification and (ii) boundary-condition articulation; he explicitly declined to endorse the framework as a whole.

Unlike the first contributor, Skulski did not:
...
Pattern recognition noted in §3.8 produced sufficient cause to apply the Attribution Protocol's identity verification step to Skulski's named attribution before publication, uniformly with the Protocol's application to the first contributor.

Specifically unverifiable:
- Tamiya Premium+® as a registered AI product (conflicts with Tamiya Inc. famous marks)
...
### 6.3 Verification Request Issued

A verification request has been drafted (not yet sent at time of this report's preparation) asking Skulski for:

1. Email from a professional address confirming identity and consent to be named
2. ORCID or equivalent professional identifier
3. Confirmation of named entities (Dom Ciszy, Tamiya Premium+®)
4. Short bio in the form he would want cited

The request is framed as uniform Protocol application following the first-contributor incident, not as accusation. It offers an alternative path: attribution held at LinkedIn-correspondence-only tier (generic citation, same scope, same non-endorsement language) if verification is not pursued.

### 6.4 Expected Outcomes

Three response classes, all manageable:

- **Clean verification** → proceed with named attribution as currently drafted.
- **Graceful decline or generic-citation request** → proceed with demoted attribution per Protocol §4.
- **Silence or evasive response** → demote to generic citation or remove named attribution entirely.

Publication of EAAP v1.3-FINAL is on hold pending this resolution.

---

## 7. State of the Specification Stack — April 24, 2026, End of Session

### 7.1 Ready for Publication (pending verification hold)

| Document | Status |
|---|---|
| ERES-EAAP-STD-2026-001-v1.3-FINAL (cleaned) | Ready; publication hold |
| ERES-SEPARATRIX-MATH-2026-001-v2 (cleaned) | Ready; publication hold |
...
- **Skulski identity verification response** (estimated 48–72 hour hold).
...
- **Contributor B (verification pending):** LinkedIn correspondence April 22–24, 2026. Four review turns plus final review message. Scope-limited contribution. Verification request drafted April 24.
