...
## v1.3-DRAFT: Four-Factor Proof-of-Resonance, Scope Clarification, Decision-Space Accessibility, and DSAP-PRE Companion Acknowledgment
...
**Revision note:** This draft has been updated (April 23, 2026) to acknowledge the pre-threshold regime-detection scope limit of DSAP as specified in Section 4, and to name DSAP-PRE as a companion specification under development (Section 8.6, ERES-SEPARATRIX-MATH-2026-001-v1).
...
DSAP-PRE (Section 8.6) is a named companion specification under development and is **not** required for v1.3-FINAL conformance. v1.3 and DSAP-PRE may proceed on independent timelines.
...
- **DSAP scope limit acknowledged.** Section 4.4 explicitly states that DSAP as specified measures degradation of decision-space accessibility but does not detect pre-threshold regime transition. Pre-threshold regime detection is addressed by DSAP-PRE (ERES-SEPARATRIX-MATH-2026-001-v1), a named companion specification under development.
...
### 4.4 Scope Limit — DSAP Measures Degradation, Not Regime Transition

Subsequent peer review (April 2026, Reviewer A per the Attribution Protocol) established that DSAP as specified here measures *degradation of decision-space accessibility* — the point at which one or more of the three path-accessibility factors (R, P, F) crosses its threshold. DSAP does **not** detect the structurally earlier moment at which a system enters a regime of irreversible path dependency while all four PoR factors still evaluate above threshold.

Pre-threshold regime-transition detection is a separate specification requirement, addressed by **DSAP-PRE** (Pre-threshold Decision Space Accessibility). DSAP-PRE uses derivative and topological analysis rather than state-and-magnitude scoring. Its mathematical scaffolding is specified in ERES-SEPARATRIX-MATH-2026-001-v1 (Separatrix Crossing Math Sheet); the full specification will follow as ERES-DSAP-PRE-SPEC-2026-001.

This limit is important to state explicitly. An implementation of EAAP v1.3 with DSAP as defined here detects decision-space collapse at the measurement point; it does not detect regime transition in advance of measurement. Deployments in high-consequence domains should plan to integrate DSAP-PRE when available.
...
### 8.6 Pre-threshold Regime Detection (DSAP-PRE)

Subsequent peer review (April 2026) established that the four-factor PoR, including DSAP as specified in Section 4, measures degradation of decision-space accessibility but does not detect the structurally earlier moment at which a system enters a regime of irreversible path dependency while all four factors still evaluate above threshold.

The pre-threshold regime-detection layer is specified separately as **DSAP-PRE** (Pre-threshold Decision Space Accessibility), with mathematical scaffolding in ERES-SEPARATRIX-MATH-2026-001-v1 (Separatrix Crossing Math Sheet). DSAP-PRE uses derivative and topological analysis (RHC second derivative, PERT distribution modality and variance, hysteresis counterfactual) rather than state-and-magnitude scoring.

DSAP-PRE is a **named companion specification** under development. It is not required for v1.3-FINAL conformance. v1.3-FINAL conformant implementations deployed in high-consequence domains should plan to integrate DSAP-PRE when the full specification (ERES-DSAP-PRE-SPEC-2026-001) is published.
...
This draft incorporates peer review contributions from two independent reviewers who engaged via LinkedIn correspondence during April 2026. Reviewers are referenced generically pending their own verification and consent to attribution, per the ERES Attribution Protocol.

- **Reviewer A (LinkedIn correspondence, April 22–23, 2026)** — contributed, across multiple correspondence turns, (i) the distinction between state consistency and path accessibility, which led directly to the four-factor Proof-of-Resonance specified in Section 3; and (ii) the distinction between measurement of degradation and detection of irreversibility-onset, which established the pre-threshold regime-detection requirement acknowledged in Section 4.4 and named as DSAP-PRE in Section 8.6.

- **Reviewer B (LinkedIn correspondence, April 23, 2026)** — contributed the scope clarification identifying EAAP as qualification and correlation rather than execution gate, and named the runtime containment layer as a distinct specification requirement (Section 8.3).

The ERES Institute has not independently verified the named identity, affiliation, or role of either reviewer beyond the LinkedIn correspondence record. Named attribution will be added in v1.3-FINAL or a subsequent revision upon reviewer request and ERES Institute confirmation.
...
- **ERES-SEPARATRIX-MATH-2026-001-v1** — Separatrix Crossing Math Sheet (DSAP-PRE mathematical scaffolding, companion work under development).
