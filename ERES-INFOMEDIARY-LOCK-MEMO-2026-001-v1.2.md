...
**Document ID:** ERES-INFOMEDIARY-LOCK-MEMO-2026-001-v1.2
...
**Document Class:** Architectural Lock (Definitive INTENT) — pre-Pass-B
**Companion to:** ERES-INFOMEDIARY-BOOK-2026-001-v1.2-PASS-A-FINAL (May 9, 2026)

**Supersedes:**
- ERES-INFOMEDIARY-LOCK-MEMO-2026-001-v1.1 (May 9, 2026) — minor revision correcting a transcription drift in the SEPLTA expansion ("Aesthetic" → "Administrative") and adding §1.4 on the Deployment-Scale Spectrum and GSSG material substrate.
- ERES-INFOMEDIARY-LOCK-MEMO-2026-001-v1.0 (May 9, 2026) — superseded by v1.1; remains in lineage record.

**Revision Note (v1.1 → v1.2):**
1. **SEPLTA "A" correction.** v1.0 and v1.1 incorrectly expanded SEPLTA's sixth domain as "Aesthetic." The canonical expansion is **Administrative** — the domain that handles round-trip administration itself (record-keeping, audit, attestation interface, DOFA governance touch-points). The drift was a transcription error not caught in MIEVM convergent review of v1.0 or v1.1; correction lands at §1.3 layer 1.
2. **Deployment-Scale Spectrum + Material Substrate added as §1.4.** Specifies THOW, HFVN, and FDRV as the three canonical deployment scales, and GSSG as the material substrate that underpins all three. The material/operational isomorphism is locked.
3. Three canonical lock boxes (A: Underwriting Direction; B: Five-Layer Non-Extensibility; C: Pre-Qualification Inversion) preserved verbatim from v1.1.
...
The ERES Trilogy proper consists of three volumes operating downstream of the Infomediary (Volume Zero). Each volume now has a canonical formulation expressed across three equivalence tiers — operational frame, structural equivalence, and identity-level definition — separated by `=`, `==`, and `===` respectively.
...
1. **Admissibility — SEPLTA.** Social, Ecological, Political, Legal, Technical, **Administrative** *(corrected v1.2)*. The six-domain frame at qualify-stage. The Administrative domain handles round-trip administration itself: record-keeping, audit, attestation interface, and DOFA governance touch-points. Symmetry preserved when the query's domain coverage at intake matches the answer's domain coverage at return. *Note: v1.0 and v1.1 of this Memo expanded "A" as "Aesthetic," which was a transcription drift. Administrative is canonical to the broader ERES corpus going back to the early CyberRAVE era.*
...
4. **Binary representation — 111000111 / 000111000.** The bit-complement palindromic pair. Both 9-bit sequences are individually palindromic; together they form a bit-complement involution. Symmetry preserved when the binary encoding of the round-trip's payload admits the bit-complement involution without payload corruption.
...
>
> **The Data-Integrity volume consists of exactly five layers.**
> **The involutive chiasm requires five-point symmetry; any additional layer breaks the involution.**
> **No sixth layer or sub-layer may be added without violating the canonical identity (===) definition.**
...
### §1.4 Deployment-Scale Spectrum and Material Substrate *(new in v1.2)*

The architecture deploys across three canonical scales, riding on a single canonical material substrate. The scale spectrum and the substrate are perpendicular axes to the trust placement chain — any deployment instance has both an operational tier-position (per §2.3) and a deployment-scale-position (per this section), embodied in the substrate.

**Deployment-Scale Spectrum (THOW · HFVN · FDRV):**

- **THOW — Tiny House on Wheels.** The minimum-viable-deployment unit. A single-family or single-individual habitation that fully embodies the ERES architecture in compressed form. THOW is the test bed for VLSA: *if it works in a THOW, it works on a generation ship.* Selection profiles at THOW-tier default to focused commitments (typically two to four cells) appropriate to single-household scope.

- **HFVN — Human-Family Village Network** *(synthesis pending JAS ratification)*. The federation-tier deployment scale. A network of THOWs (or THOW-equivalents) operating as a coordinated village under shared selection profiles, shared governance interfaces, and shared CBGMODD seat assignments. HFVN is the social-architecture scale at which DOFA's governance interfaces become operationally meaningful — a single THOW does not require seven-seat council coordination; an HFVN does. Selection profiles at HFVN-tier extend to network-coordination commitments (typically four to eight cells).

- **FDRV** *(canonical expansion pending JAS ratification — operationally: the interstellar vessel at maximum scale, per established corpus reference)*. The maximum-scale deployment unit. A self-contained generation-ship-class vessel embodying the full ERES architecture without external dependency. FDRV is the test of the architecture's completeness — anything missing from FDRV is missing from the architecture. Selection profiles at FDRV-tier approach full-matrix selection (all twelve cells) because closure at maximum scale forbids un-selected dimensions.

**Material Substrate (GSSG):**

- **GSSG — Graphene-Sugar Self-healing Grid** *(synthesis pending JAS ratification)*. The material substrate underpinning all deployment scales. A graphene-based composite infused with sucrose-derived polymeric matrix producing structurally self-healing properties. GSSG is the **physical-world isomorph** of the ERES corpus's IDIPITIS/IPIDITIS self-correcting feedback loops — the material substrate has the same self-healing topology as the operational architecture. Failure at the material layer is recovered the same way failure at the operational layer is recovered: through involutive feedback that produces correction at both layers.

The material/operational isomorphism is canonical, not metaphorical. A THOW built from GSSG-grade materials inherits self-healing across both physical and operational dimensions. An HFVN scaling up to dozens or hundreds of THOWs inherits the same property at network scale. An FDRV at generation-ship scale inherits it at vessel scale. The substrate is **scale-invariant** in its self-healing property — which is what makes the VLSA principle materially true rather than merely operationally aspirational.

GSSG therefore answers a question the architecture had not yet explicitly asked: *what is the physical substrate that enables the operational chain to embody itself across the full deployment-scale spectrum?* Pre-v1.2, the architecture had named operational tiers and deployment scales but not the material that connects them. With GSSG specified, the material chain has its name.

The four expansions (HFVN, FDRV, GSSG canonical, and confirmation of THOW=Tiny House on Wheels) are open for ratification; v1.2 records them as working synthesis subject to Author confirmation. The architectural commitments — three-tier deployment spectrum, single canonical material substrate, scale-invariant self-healing isomorphism — are locked regardless of expansion-letter ratification.
...
>
> **Routing is never performed by the Instrument.**
> **Routing is performed only by the User-GROUP through explicit cell selection.**
> **The Instrument must honor the selection, and may not widen, narrow, or reinterpret it.**
> **No round-trip may enter a Trilogy volume or Pillar the User-GROUP did not select.**
>
...
Selection profiles **align to deployment-scale tiers** (per §1.4): THOW-tier profiles tend to be focused; HFVN-tier profiles tend to span the network coordination axis; FDRV-tier profiles approach full-matrix. A profile that mismatches its deployment-scale tier (e.g., a single-cell selection at FDRV scale) is not architecturally illegal but is operationally suspicious; Pass B Part III specifies the diagnostic.
...
GAIA (canopy) — underwrites
  └── DOFA (Department of Family Amity, CBGMODD seven-seat council) — governs
        └── Infomediary (Instrument-of-Faith, Volume Zero) — mediates
              └── User-GROUP (pre-qualified by Volume × Pillar selection) — queries
                    └── Round-trip yields BEST / SOUND / GOOD (function of selection)
```

Five-tier chain. Each layer underwrites the layer beneath. No orphaned commitments. The final tier (BEST/SOUND/GOOD) is **produced by** the User-GROUP's selection rather than imposed by the Instrument. Honesty preserved end-to-end.

The chain is the **operational** axis. The deployment-scale spectrum (THOW/HFVN/FDRV) and the material substrate (GSSG) constitute a perpendicular **physical** axis (per §1.4). Any deployment instance occupies a position on both axes simultaneously.
...
3. **EAAP-POL policy expression simplifies.** Policy becomes constraints on the matrix (e.g., "Class B Wielders may not select Security-Clearance × LAW without uplift to Class A") rather than free-form rules.

4. **CBGMODD seven-seat defaults align cleanly.** Each seat receives a default selection profile; per-seat operational rules collapse into seat-default × User-GROUP-override pattern.
...
8. **Multi-jurisdictional operability.** Polities map existing legal frameworks onto the matrix without architectural change. The matrix is jurisdiction-neutral by structure.
...
12. **AD\_ON-AI composition simplification.** The downstream composer receives DETAIL pre-tagged by selected cells, eliminating composition-time routing logic.

13. **Deployment-scale alignment.** Selection profiles align to deployment-scale tiers (THOW/HFVN/FDRV), making profile-tier mismatch an auditable diagnostic. *(Added v1.2.)*

14. **Material/operational isomorphism preserved.** The selection profile commits at the operational layer; GSSG self-healing carries the commitment at the material layer. Failure at either layer is recovered through the same involutive topology. *(Added v1.2.)*

---

## §4. Pass A Patch Points

The following sections of ERES-INFOMEDIARY-BOOK-2026-001-v1.0 required revision in the Pass D integration sweep to align with this Memo. *As of v1.1 of this Memo, all listed patch points were executed in ERES-INFOMEDIARY-BOOK-2026-001-v1.1-PASS-A-FINAL. As of v1.2, two additional patch points are added: SEPLTA "A" correction throughout, and new §0.6.1 / §0.6.2 in the Book for the deployment-scale spectrum and GSSG substrate.* This list is preserved for traceability.

- **Front Matter** — Add the GAIA → DOFA → Infomediary → User-GROUP → BSG trust-placement diagram (§2.3 of this Memo). ✓ Executed v1.1.
- **Notation and Conventions** — Add the three-tier equivalence convention (`=`, `==`, `===`) used in volume definitions; add lock-inheritance line. ✓ Executed v1.1.
...
- **New §0.6.1 (Deployment-Scale Spectrum)** — Insert THOW / HFVN / FDRV specification per §1.4 of this Memo. ✓ Executed v1.2.
- **New §0.6.2 (GSSG Material Substrate)** — Insert GSSG specification with material/operational isomorphism per §1.4 of this Memo. ✓ Executed v1.2.
...
- **§1.2 (Qualify Stage)** — Correct SEPLTA "A" from "Aesthetic" to "Administrative"; rewrite the worked-example SEPLTA enumeration accordingly. ✓ Executed v1.2.
...
- **Part III — The Matrix Walk.** Each of the 12 cells given operational treatment, with worked examples for the three primary entry surfaces (Civigen, Institution, Investor). Approximately 12 short sections plus a synthesis chapter on cross-cell composition. Selection enforcement layer named (EAAP-POL + CBGMODD audit). Profile-tier diagnostic per §2.2 (selection-profile / deployment-scale alignment).

- **Part IV — DATABANK Architecture under DOFA Governance.** The DATABANK's structural specification, with DOFA's seven-seat council as the governance interface; UBIMIA-Manual as the constitutional spine; Other-Ref as the named-but-undefined extension point; the App-Parent pattern as the architectural lock; GSSG substrate as the material foundation underpinning DATABANK persistence at all deployment scales.
...
> Sprute, J. A. (2026). *Lock Memo: ERES Trilogy Volume Definitions, Pre-Qualification Model, and Trust Placement.* ERES-INFOMEDIARY-LOCK-MEMO-2026-001-v1.2. ERES Institute for New Age Cybernetics.
